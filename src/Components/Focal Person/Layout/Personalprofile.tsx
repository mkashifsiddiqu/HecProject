...
export default function BasicMenu() {
  ...
  return (
    <>
    <div>
      <Button disableRipple onClick={handleClick} sx={{textTransform: `none`, '&:hover': { backgroundColor: `transparent` } }}><Typography display={`flex`} alignItems={`center`} fontFamily={`montserrat`} fontWeight={600} fontSize={13} color="#636177">Greetings, Focal Person<Avatar sx={{ marginLeft: `0.55rem` }}>F</Avatar></Typography></Button>
      <Menu
        id="basic-menu"
        anchorEl={anchorEl}
        open={open}
      ...
      <Box display={`flex`} alignItems={`center`} onClick={handleClose} sx={{ padding: `25px 102px 30px 30px`, backgroundImage: `linear-gradient(45deg, #17a667 10%, #0a8782 90%)` }}>
          ...
          <Box padding={`0 0 0 15px`}>
            <Typography fontFamily={`montserrat`} fontSize={15.6} fontWeight={900} color="#fff">Focal Person</Typography>
            <Typography fontFamily={`montserrat`} fontSize={13} fontWeight={500} color="#fff">FP@hec.com</Typography>
          </Box>
        </Box>
